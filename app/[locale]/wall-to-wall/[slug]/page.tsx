...
export async function generateMetadata({params}:{params:Promise<{locale:string;slug:string}>}){
 ...
 const t=wtwText[locale];return pageMetadata(locale,`/wall-to-wall/${slug}`,`${model.name[locale]} | ${model.code}`,`${model.name[locale]}. ${t.intro}`,model.representativeImage);
}
export default async function WTWProductPage({params}:{params:Promise<{locale:string;slug:string}>}){
 ...
 const t=wtwText[locale];return <main id="main-content" className="wtw-product-page"><nav className="wtw-breadcrumb site-shell" aria-label="Breadcrumb"><a href={`/${locale}/wall-to-wall`}>{t.back}</a><span aria-hidden="true">/</span><bdi aria-current="page">{model.name[locale]}</bdi></nav><WTWDetail locale={locale} model={model}/></main>;
}
