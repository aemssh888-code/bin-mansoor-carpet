...
export function WTWDetail({locale,model}:{locale:Locale;model:WTWModel}){
 ...
 const [quantity,setQuantity]=useState(String(productOrderRules['wall-to-wall'].minOrderM2PerItem));
 ...
 return <>
  <div className="wtw-detail site-shell">
   ...
   <aside className="wtw-detail-info">
    ...
    {!valid&&<p id="wtw-quantity-error" role="alert" className="quantity-error">{moq.moqValidation}</p>}
    <p id="wtw-minimum-note" className="wtw-no-moq">{moq.moq}</p>
