...
export function QuoteForm({locale,products}:{locale:Locale;products:Product[]}) {
 ...
 function submit(event:SyntheticEvent<HTMLFormElement,SubmitEvent>){
  ...
  if(invalidItem){setReady('');setError(`${t.invalidDesign} ${invalidItem.modelCode}. ${invalidItem.productLine==='rug'?t.moqValidation:wtwText[locale].quantityError}`);return;}
 ...
 return <form className="quote-form" onSubmit={submit}>
  <p className="text-sm text-black/65 sm:col-span-2">{wtwText[locale].rugNav}: {t.moq}. {wtwText[locale].nav}: {wtwText[locale].noMoq}</p>
