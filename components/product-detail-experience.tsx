  ...
  return <>
    <div className="product-experience">
      ...
      <aside className="product-information">
        ...
        <label className="quantity-field">{t.quantity}<input type="number" min="0" step="any" value={quantity} onChange={event=>setQuantity(event.target.value)}/></label>
        <div className="product-actions"><button type="button" className="btn-primary" onClick={addCurrent}>{added?t.added:t.add}</button><a href={`/${locale}/quote`} onClick={addCurrent} className="btn-secondary">{t.quote}</a><a href={direct} target="_blank" rel="noreferrer" className="btn-text">WhatsApp</a></div>
        <output className="sr-only">{added?t.added:''}</output>
        <dl className="subdued-specs"><div><dt>{t.factoryRef}</dt><dd><bdi>{product.originalCode}</bdi></dd></div><div><dt>{t.collection}</dt><dd>{product.collection[locale]}</dd></div><div><dt>{t.style}</dt><dd>{product.styles.map(style=>style[locale]).join(' · ')}</dd></div>{product.availability.minimumOrderQuantity!==null&&<div><dt>MOQ</dt><dd>{t.moq}</dd></div>}</dl>
    ...
    <div className="mobile-product-actions"><button type="button" onClick={addCurrent}>{added?t.added:t.add}</button><a href={direct} target="_blank" rel="noreferrer">WhatsApp</a></div>
  </>;
}
