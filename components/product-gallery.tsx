...
export function ProductGallery({product,locale}:{product:Product;locale:Locale}) {
 ...
 function addCurrent(){if(!valid)return;add({productLine:'rug',modelCode:product.binMansoorCode,name:product.name,colorCode:active.code,colorName:active.name,quantity,image:active.image,route:`/${locale}/products/${product.slug}`});setAdded(true);window.setTimeout(()=>setAdded(false),1800);}
